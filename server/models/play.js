...
module.exports = (Play) => {
  ...
  //   });
  //   try {
  //     let player = await Player.findOne({ where: { name: data.name } });
  //     if (_.isNil(player)) {
  //       player = {
  //         name: data.name,
  //         playCount: 1,
  ...
  //     player.playCount += 1;
  //     player = await Player.upsert(player, { transaction: tx });
  //     const playInput = {
  //       submitTime: moment().utc(),
  //       score: data.score,
  //     };
  //     const play = await player.plays.create(playInput, { transaction: tx });
  //     await tx.commit();
